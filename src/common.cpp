...
uint32_t load_external_texture(void* image_data, uint32_t dataSize, struct texture_set *texture_set, struct tex_header *tex_header, uint32_t originalWidth, uint32_t originalHeight, uint32_t saveload_palette_index)
{
	...
	if((uint32_t)VREF(tex_header, file.pc_name) > 32 && !save_textures)
	{
		...
		{
			bool isInZzz = remastered_edition && strncmp(VREF(tex_header, file.pc_name) + 512, "zzz://", 6) == 0;

			// Retry with hd name outside zzz archive
			if (texture == 0) {
				texture = load_texture(image_data, dataSize, VREF(tex_header, file.pc_name) + 512 + (isInZzz ? 6 : 0), saveload_palette_index, VREFP(texture_set, ogl.width), VREFP(texture_set, ogl.height), gl_set);
			}

			// Retry inside zzz archive for the remastered
			if (remastered_edition && texture == 0 && isInZzz) {
				texture = load_texture(image_data, dataSize, VREF(tex_header, file.pc_name) + 512, saveload_palette_index, VREFP(texture_set, ogl.width), VREFP(texture_set, ogl.height), gl_set);
			}
		}
	}
