...
struct ff8_tex_header *ff8_load_tex_file(struct ff8_file_context* file_context, char *filename)
{
	...
	if (ret->file.pc_name != nullptr) {
		...
		if (g_FF8ZzzArchiveMain.fileExists(remasterFileName)) {
			strncpy(ret->file.pc_name + 512, remasterFileName, strlen(remasterFileName) - 4);
		} else {
			// Paletted + lang path
			_snprintf(remasterFileName, sizeof(remasterFileName), "%s\\%s\\0.png", suffix, langPath);
			if (g_FF8ZzzArchiveMain.fileExists(remasterFileName)) {
				strncpy(ret->file.pc_name + 512, remasterFileName, strlen(remasterFileName) - 6);
			} else {
