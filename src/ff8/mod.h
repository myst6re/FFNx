...
public:
	TextureImage();
	bool createImage(const char *filename, int originalTexturePixelWidth, int originalTextureHeight);
	void destroyImage();
...
private:
	...
	uint8_t computeLod(int originalTexturePixelWidth) const;
...
class ModdedTexture {
public:
	ModdedTexture(const TexturePacker::IdentifiedTexture &originalTexture);
	...
	inline virtual bool drawnOnce(int vramPalXBpp2, int vramPalY) const {
		return _drawn;
	}
...
protected:
	...
	inline void setDrawnOnce(bool drawn) {
		_drawn = drawn;
	}
private:
	const TexturePacker::IdentifiedTexture &_originalTexture;
	bool _drawn;
};
...
public:
	...
	bool drawnOnce(int vramPalXBpp2, int vramPalY) const override;
...
struct TextureRedirection : public ModdedTexture {
	TextureRedirection(
		const TexturePacker::IdentifiedTexture &originalTexture,
		const TexturePacker::TextureInfos &newTexture
	);
	TextureRedirection(const TextureRedirection &other) = delete;
	~TextureRedirection();
	...
	bool createImages(uint32_t *imageData);
	inline uint32_t *imageData() {
		return _image;
	}
	inline const TexturePacker::TextureInfos &newTexture() const {
		return _newTexture;
	}
	TexturePacker::TextureTypes drawToImage(
		int offsetX, int offsetY,
...
private:
	...
	uint32_t *_image;
	TexturePacker::TextureInfos _newTexture;
	uint8_t _scale;
};
