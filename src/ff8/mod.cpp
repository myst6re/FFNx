...
bool TextureImage::createImage(const char *filename, int originalTexturePixelWidth, int originalTextureHeight)
{
	if (_image != nullptr)
	{
		destroyImage();
	...
	setLod(computeLod(originalTexturePixelWidth));
...
uint8_t TextureImage::computeLod(int originalTexturePixelWidth) const
{
	...
	int maxScale = computeMaxScale(),
		maxWidth = originalTexturePixelWidth * maxScale,
...
ModdedTexture::ModdedTexture(const TexturePacker::IdentifiedTexture &originalTexture):
	_originalTexture(originalTexture), _drawn(false)
{
}
...
{
	ffnx_trace("ModdedTexture::%s: sourceRgbaW=%d sourceScale=%d targetRgbaW=%d targetScale=%d sourceX=%d sourceY=%d sourceW=%d sourceH=%d targetX=%d targetY=%d\n",
		__func__, sourceRgbaW, sourceScale, targetRgbaW, targetScale, sourceX, sourceY, sourceW, sourceH, targetX, targetY);
	...
	int sourceXBpp2, int sourceY, int sourceWBpp2, int sourceH, int targetXBpp2, int targetY)
{
	...
	ffnx_trace("ExternalTexture::%s: w=%d scale=%d "
			"sourceX=%d sourceY=%d sourceW=%d sourceH=%d targetX=%d targetY=%d\n",
			__func__, targetRgbaW, scale, sourceX, sourceY, sourceW, sourceH, targetX, targetY
		);
...
bool TextureModStandard::createImages()
{
	int paletteCount = originalTexture().palette().h(); // Works most of the time
	bool hasPal = paletteCount > 0 || originalTexture().texture().bpp() != Tim::Bpp16;
	...
	for (int paletteId = 0; paletteId < modCount; ++paletteId) {
		if (!findExternalTexture(originalTexture().name().c_str(), filename, paletteId, hasPal, extension, found_extension))
		{
			continue;
		}
		...
		if (!externalTexture.createImage(filename, originalTexture().texture().pixelW(), originalTexture().texture().h()))
		{
			continue;
		}
...
{
	if (trace_all || trace_vram) ffnx_trace("TextureModStandard::%s vramPal=(%d, %d) original=(%d, %d)\n", __func__, vramPalXBpp2, vramPalY, originalTexture().palette().x(), originalTexture().palette().y());
...
{
	if (trace_all || trace_vram) ffnx_trace("TextureModStandard::%s vramPal=(%d, %d)\n", __func__, vramPalXBpp2, vramPalY);

	return textureImage(computePaletteId(vramPalXBpp2, vramPalY)).scale();
}

bool TextureModStandard::drawnOnce(int vramPalXBpp2, int vramPalY) const
{
	return ModdedTexture::drawnOnce(vramPalXBpp2, vramPalY) && _lastDrawnPalette == computePaletteId(vramPalXBpp2, vramPalY);
}
...
{
	if (drawnOnce(vramPalXBpp2, vramPalY))
	{
		ffnx_trace("Already drawn once\n");
		return TexturePacker::RemoveTexture;
	}

	const TexturePacker::TextureInfos &origTexture = originalTexture().texture();

	ffnx_trace("TextureModStandard::%s: offsetX=%d offsetY=%d targetW=%d targetH=%d targetScale=%d targetBpp=%d vramPalXBpp2=%d vramPalY=%d origTextureBpp=%d origTexturePixelX=%d origTextureY=%d\n",
		__func__, offsetX, offsetY, targetW, targetH, targetScale, targetBpp, vramPalXBpp2, vramPalY, origTexture.bpp(), origTexture.pixelX(), origTexture.y());
	...
	ffnx_trace("Drawn once? %d %d %d %d\n", width, origTexture.pixelW(), height, origTexture.h());

	if (width == origTexture.pixelW() && height == origTexture.h())
	{
		setDrawnOnce(true);
		_lastDrawnPalette = paletteId;
	}
...
bool TextureBackground::createImages(const char *extension, char *foundExtension)
{
	...
	for (const Tile &tile: _mapTiles) {
		...
		if (_vramPageId < 0 || _vramPageId == texId) {
			...
			const uint16_t key = uint16_t(texId) | (uint16_t(bpp) << 4);
			_usedBpps |= 1 << bpp;
...
TexturePacker::TextureTypes TextureBackground::drawToImage(
	...
	int16_t vramPalXBpp2, int16_t vramPalY)
{
	const bool withTextureId = _vramPageId >= 0;
	const uint8_t textureId = withTextureId ? _vramPageId : -offsetX / (4 >> uint16_t(targetBpp)) / TEXTURE_WIDTH_BPP16;
	const uint16_t key = uint16_t(textureId) | (uint16_t(targetBpp) << 4);
	...
	uint32_t imgWidth = mip.m_width;
	uint8_t imgScale = _texture.scale();
	std::set<uint32_t> matched;

	if (trace_all || trace_vram) ffnx_trace("%s: imgWidth=%d imgScale=%d\n", __func__, imgWidth, imgScale);
	...
	for (const std::pair<uint16_t, size_t> &pair: std::ranges::subrange{begin, end}) {
		...
		uint8_t targetX = tile.srcX, targetY = tile.srcY;

		if (matched.contains(targetX | (uint32_t(targetY) << 16))) {
			if (trace_all || trace_vram) ffnx_warning("TextureBackground::%s: multiple tiles found for the same position (tile id %d)\n", __func__, tileId);

			break;
		}

		matched.insert(targetX | (uint32_t(targetY) << 16));
...
TextureRedirection::TextureRedirection(
	const TexturePacker::IdentifiedTexture &originalTexture,
	const TexturePacker::TextureInfos &newTexture
) : ModdedTexture(originalTexture), _image(nullptr), _newTexture(newTexture),
	_scale(computeScale())
{
}
...
bool TextureRedirection::createImages(uint32_t *imageData)
{
	_image = imageData;
...
uint8_t TextureRedirection::computeScale() const
{
	return _newTexture.h() / originalTexture().texture().h();
}

TexturePacker::TextureTypes TextureRedirection::drawToImage(
	int offsetX, int offsetY,
	uint32_t *targetRgba, int targetW, int targetH, uint8_t targetScale, Tim::Bpp targetBpp,
	int16_t vramPalXBpp2, int16_t vramPalY)
{
	...
	ffnx_trace("TextureRedirection::%s: offsetX=%d offsetY=%d targetW=%d targetH=%d targetScale=%d targetBpp=%d origTextureBpp=%d origTexturePixelW=%d origTextureH=%d\n",
		__func__, offsetX, offsetY, targetW, targetH, targetScale, targetBpp, origTexture.bpp(), origTexture.pixelW(), origTexture.h());
	...
	drawImage(
		_image, _newTexture.pixelW() / _scale, _scale,
		targetRgba, targetW, targetScale,
		sourceX, sourceY, width, height,
		targetX, targetY
	);
