...
uint32_t ff8_wm_section_38_prepare_texture_for_upload(uint8_t *tim_file_data, ff8_tim *tim_infos)
{
	...
	uint8_t worldmap_without_textured_anim = *(uint8_t *)0x1F9D916;

	if (!worldmap_without_textured_anim)
	{
		if (timId == 0)
		{
			// Open section 17
			uint8_t *wm_section_17_dword_2040068 = *(uint8_t **)0x2040068;
			uint8_t *wm_section_18_dword_2040638 = *(uint8_t **)0x2040638;
	...
	uint8_t worldmap_without_color_anim = *(uint8_t *)0x1F9D915;

	if (!worldmap_without_color_anim)
	{
		if (timId == 0)
		{
			// Open section 41
			uint8_t *wm_section_41_animation_palettes_dword_203FAF8 = (uint8_t *)*ff8_externals.worldmap_section41_position;
			uint8_t *wm_section_42_dword_2040690 = (uint8_t *)*ff8_externals.worldmap_section42_position;
	...
	for (const WmsetSection17Texture &texture: wm_wmset_wave_animations_textures) {
		...
		for (const uint8_t *frameTexturePos: texture.textureFramePositions) {
			if (frameTexturePos == tim_file_data) {
				...
				((void(*)(uint8_t*,int16_t,int16_t))0x5416A0)(tim_file_data, x, y);
...
void vram_init()
{
	...
	replace_call(0x554C90 + 0xCC, ff8_wm_section_41_upload_palette); // Animated palettes
